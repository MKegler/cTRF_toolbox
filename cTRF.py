...
def ridge_fit_SVD(XtX, XtY ,lambdas):
    ...
    Notes:
    1. For forward model, the output matrix is rectangular [timelags x channels]
    but for backward model it is a row vector of length [timelags*channels].
    2. To obtain the rectangular shape, each of those row vectors need to be
...
def design_martix(eeg, feat, tlag, cmplx=True, forward=False, normalize=True):
    '''
...
def train(eeg_list, feat_list, tlag, cmplx=True, forward=False, lambdas=[0], normalize=True):
    ...
    Takes training eeg, fundamental waveform (Y) datasets zscores and fits the
    backward or forward model.
    ...
    for (eeg, feat) in zip(eeg_list, feat_list):
        X_tmp, Y_tmp = design_martix(eeg, feat, tlag, cmplx, forward, normalize)
        X.append(X_tmp)
        Y.append(Y_tmp)
